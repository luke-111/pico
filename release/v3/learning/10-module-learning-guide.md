...
这篇我不先罗列目录，而是先问“这个模块为什么要存在”。读 Pico 之前先抓住最小 Agent 的四件事，再看我在 v3 里怎么把它们扩展成一个本地 coding agent runtime harness。
...
所以别从“文件很多”开始读，从下面 8 个问题开始。

## 1. Agent loop：一次请求怎么推进

最小 Agent 里，`agent.py` 往往直接写一个 ReAct 循环：调模型、判断有没有工具、执行工具、把结果塞回上下文。
...
先看 `Pico.ask()`，它很薄，只把请求交给 `Engine`。真正的控制流在 `Engine.run_turn()`。这里能看出我的核心取舍：`Pico` 持有状态，`Engine` 推进状态。
...
## 2. Context：为什么不是简单的 messages list

最小 Agent 的 context 一般就是消息列表加一个裁剪函数。我的 context 要处理更多来源：系统前缀、工作记忆、durable memory、skills、相关记忆、历史、当前请求、todo、runtime mode。
...
第一，prompt 不是一段字符串，是一组 section。每段有预算，超出时按规则裁剪。我不追求无限上下文，而是让不同信息有不同寿命。

第二，context metadata 会写进 run report。prompt 构建不是黑盒，事后能复盘“模型当时为什么看到这些信息”。
...
最小 Agent 的 tools 往往就是一个 registry：工具名、schema、Python 函数。我这边的工具边界厚得多，因为 coding agent 会真的读写仓库、跑 shell、改文件。
...
| `pico/core/tool_policy.py` | read-before-write、不要用 shell 做搜索和读文件等行为策略。 |
| `pico/core/tool_repetition.py` | 重复工具调用保护。 |
| `pico/features/sandbox/` | `run_shell` 的 bubblewrap 和降级执行层。 |
...
关键认知是：工具不是“模型可以调用的函数”，而是“可审计的外部动作”。v3 的很多修复都在这一层，比如 shell policy 对管道后 `head/tail/grep` 的处理、写文件前必须 fresh read、plan mode 的写边界。
...
最小 Agent 的常见做法是把历史都放进 messages。我在 v3 里把记忆分两条线：session 内的 working memory 和跨 session 的 durable memory。
...
我的核心取舍是：不把所有历史塞回 prompt，而是把稳定事实提升为 durable topic，把临时上下文留在 session 里。auto-dream 是后台整理器，主循环不用等它。
...
最小 Agent 一般只支持一个模型 API。我支持 OpenAI-compatible、Anthropic-compatible 和 DeepSeek profile，但 runtime 不该知道这些协议细节。
...
这里的学习重点不是“怎么调 API”，是看 provider 层怎么保护 runtime：

- runtime 只调 `complete(prompt, max_new_tokens, ...)`。
- provider 负责 HTTP、重试、usage、cache metadata、错误分类。
- CLI 负责把配置 profile 装成具体 client。
- trace 和 report 里不能泄漏 key，所以 secret redaction 是 provider 的配套能力。
...
交互层会改变系统形态。CLI、slash command、skills、TUI 都不是主循环，但它们决定用户怎么驱动主循环。
...
| `pico/tui/app.py` | Textual app，驱动同一条 runtime event stream。 |
...
- skill 是 prompt-driven workflow，不给 agent 新工具，只告诉 agent 怎么组合已有工具。

我的 TUI 不重新实现 agent loop。它调同一个 runtime，把 runtime events 渲染成消息、工具卡片、审批框和状态栏。原则是：UI 层事件驱动，但不能抢走 runtime 的决策权。
...
最小 Agent 只有一个循环。复杂任务需要计划、任务账本和子 agent，否则探索、修改、验证都会挤进同一个上下文。
...
1. 先看 `PlanModeManager.enter()`，理解 plan mode 不是提示词，是 runtime mode 加 tool profile。
2. 再看 `TodoLedger`，理解 todo 会进 prompt，也会写 task state。
3. 最后看 `WorkerManager.spawn()`，理解子 agent 是受限 child runtime，不是另一个没有边界的 agent。
...
最小 Agent 一般只返回一段最终文本。我在 v3 的目标不是“看起来回答了”，而是能复盘：这次 run 做了什么、改了什么、为什么停。
...
`task_state` 回答“现在到哪一步”，`trace` 回答“每一步发生了什么”，`report` 回答“最后怎么收口”。v3 的真人场景测试就是围绕这些证据文件做验收。
...
| 记忆不就是把历史塞回去吗？ | `03-context-memory-compact.md`、本篇第 4 节 |
| 多模型怎么接？ | `06-providers-config.md`、本篇第 5 节 |
| 子 agent 怎么保证不乱跑？ | `05-workers-plan-todo.md`、本篇第 7 节 |
| 怎么证明它真的可用？ | `08-session-run-evaluation.md`、`release/v3/TESTING.md`、本篇第 8 节 |

## 一句话总结

这条学习路线没有停在 minimal loop，而是从 minimal loop 长出一个 runtime harness。前者解决“Agent 怎么跑起来”，后者解决“Agent 怎么在真实仓库里可控、可恢复、可复盘地跑下去”。
